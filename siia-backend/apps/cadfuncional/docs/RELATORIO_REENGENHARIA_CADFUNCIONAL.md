# Relatorio de Reengenharia - Integracao CadFuncional no Ecossistema SIIA

## Contexto confirmado
- Codigo-fonte original do CadFuncional nesta maquina: `D:\BigData\Projetos\cadfuncional`.
- Banco de dados alvo: `siia`.
- Schema do dominio CadFuncional no PostgreSQL: `cadfuncional`.

## Diagnostico consolidado

### 1) Frontend SIIA atual (workspace)
- O Launchpad e layout autenticado foram evoluidos para visual inspirado no CadFuncional.
- A rota `/dashboard/cadfuncional/*` ja renderiza paginas reais do modulo CadFuncional (fase incremental), sem bootstrap independente.
...
### 2) CadFuncional original (projeto externo)
...
- O modulo CadFuncional copiado no SIIA ainda esta no formato de app independente (router/providers bootstrap proprios).

### 3) Gap principal de integracao
- Ha divergencia de contrato entre:
  - SIIA atual: `/api/usuarios/*` (token)
  - CadFuncional original: `/api/v1/auth/*`, `/api/v1/saude/*`, `/api/v1/pessoal/*` (session/csrf + api de dominio)
- O backend `apps/cadfuncional` no SIIA ainda esta minimo (health + modelo simplificado), sem endpoints de dominio clinico.

## Decisoes arquiteturais recomendadas

### Decisao A (recomendada): SIIA como plataforma unica, CadFuncional como modulo interno
1. Manter frontend unico (`siia-frontend`) e backend unico (`siia-backend`).
2. Integrar o modulo CadFuncional como sub-rotas reais em `/dashboard/cadfuncional/*`.
3. Adaptar servicos do CadFuncional para o cliente API compartilhado do SIIA.
4. Expor no backend SIIA os endpoints de dominio CadFuncional em namespace estavel (preferencia: `/api/cadfuncional/*` com aliases temporarios `/api/v1/*` durante transicao).
...
1. Definir mapa endpoint-a-endpoint (origem CadFuncional -> destino SIIA).
2. Definir politica de autenticacao unica (token ou sessao) para todo ecossistema.
3. Configurar schema `cadfuncional` no backend SIIA para migracoes e queries.

### Fase 2 - Backend CadFuncional no SIIA
...
4. Criar migracoes reversiveis no schema `cadfuncional`.

### Fase 3 - Frontend CadFuncional integrado
1. Remover bootstrap independente do modulo copiado (sem BrowserRouter proprio).
2. Integrar providers do CadFuncional ao runtime principal (ou adapter por modulo).
3. Substituir chamadas de auth/session por estrategia escolhida no SIIA.
4. Ativar paginas reais nas rotas `/dashboard/cadfuncional/*`.

### Fase 4 - Padrao visual unico (requisito do projeto)
1. Consolidar shell visual CadFuncional como base compartilhada de apps.
...
Para usar o schema `cadfuncional` dentro do banco `siia`, configurar o `search_path` no backend SIIA, por exemplo:
- `OPTIONS: { options: '-c search_path=cadfuncional,public' }`

Observacoes:
- Essa estrategia permite que migracoes e tabelas do dominio CadFuncional sejam criadas no schema `cadfuncional`.
...
- Risco: duplicacao de routers/providers no frontend.
  - Mitigacao: converter modulo CadFuncional para modo embed no Router principal.
...
1. Login unico no SIIA acessa modulo CadFuncional sem novo login.
2. Rotas reais do CadFuncional funcionam em `/dashboard/cadfuncional/*`.
3. Endpoints criticos de saude/pessoal/estatistica respondem no backend SIIA.
4. SIAGG, CMS e CadFuncional usam o mesmo shell visual base.
...
1. Matriz endpoint-a-endpoint gerada em [MATRIZ_ENDPOINTS_CADFUNCIONAL_X_SIIA.md](MATRIZ_ENDPOINTS_CADFUNCIONAL_X_SIIA.md).
2. Backend SIIA ajustado para `search_path=cadfuncional,public` via `POSTGRES_SEARCH_PATH`.
3. Rota `/dashboard/cadfuncional/*` saiu do placeholder e passou a renderizar o dashboard real do modulo CadFuncional (fase inicial).
...
7. Adapter de autenticacao por sessao implementado para contrato CadFuncional em `/api/v1/auth/*` com os endpoints:
...
11. Endpoint critico de dominio `GET/POST /api/v1/saude/atendimentos/` implementado com shape compativel ao frontend CadFuncional.
...
19. Refatoracao de layout autenticado concluida para padrao unico de plataforma:
  ...
  - wrappers visuais redundantes removidos de CadFuncional, SIAGG, CMS e Legados (mantendo regras de acesso e providers)
...
22. Smoke tecnico de rotas do shell autenticado executado em `vite preview` com retorno HTTP `200` para:
  ...
  - `/dashboard/cms`
  - `/dashboard/legados`
23. Identificacao e aplicacao da paleta oficial do CadFuncional no shell autenticado:
  - `src/assets/css/input.css` do CadFuncional original validado como vazio (nao e fonte de estilo)
...
25. Smoke visual automatizado do shell autenticado executado com Playwright:
  ...
  - rotas verificadas: `/dashboard`, `/dashboard/cadfuncional`, `/dashboard/siagg`, `/dashboard/cms`, `/dashboard/legados`
...
26. Validacao automatizada da navegacao contextual do menu lateral por modulo:
  ...
  - CadFuncional: exibicao de menu funcional (ex.: `Médico`, `Fisioterapia`, `S-RED`) e navegacao para `/dashboard/cadfuncional/medico`
  - CMS: exibicao de menu funcional (ex.: `Notícias`, `Homologação`, `Configuração Visual`) e navegacao para `/dashboard/cms/homologacao`
  - resultado: `4 passed`
27. Correcao do menu lateral do CadFuncional para aderencia ao padrao visual/funcional do original:
...
28. Padronizacao e validacao do botao de retorno do shell autenticado:
  ...
  - validacao automatizada adicionada para as rotas iniciais `/dashboard/cadfuncional`, `/dashboard/siagg`, `/dashboard/cms` e `/dashboard/legados`
...
2. Executar validacao integrada com o frontend CadFuncional consumindo os endpoints de auth e saude ja entregues.
...
4. Consolidar frente de UX/layout no shell autenticado (`AppLayout` + `InternalAppLayout`) com foco em area util:
  ...
  - executar smoke visual em CadFuncional, SIAGG, CMS e Legados para garantir que a reducao do menu nao degrade navegacao, leitura e formularios.
